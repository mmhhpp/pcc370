	...
	char *ic;
{
	...
	extern char Peekkey_, *genindent();
...
gogo:
			...
			case 'L':
				...
				markDOT();
				goto vdown;
			case CTRL_D:	/* scroll half-screen */
				if (hadcnt)
					vSCROLL_ = cnt;
				cnt = vSCROLL_;
				if (visual) {
		...
		switch (c) {
			...
			case 'o':	/* open new lines below */
				...
				if (!visual) {
					...
					c = vcline < 0 ? ZERO_ : vliny[vcline] + vdepth();
					i = vliny[vcline + 1] - c;
					if (i < cnt)
						vopenup(cnt - i);
				}
