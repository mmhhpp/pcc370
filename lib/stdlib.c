...
void (*__userExit[__NATEXIT])();

__PDPCLIB_API__ char *malloc ( size ) size_t size ;
{
#ifdef __OS2__
    PVOID BaseAddress;
    ULONG ulObjectSize;
...
__PDPCLIB_API__ char *calloc ( nmemb,size ) size_t nmemb ;  size_t size ;
{
    void *ptr;
    size_t total;
...
__PDPCLIB_API__ char *realloc ( ptr,size ) void  ptr ;  size_t size ;
{
    char *newptr;
    size_t oldsize;
...
__PDPCLIB_API__ void free ( ptr ) void  ptr ;
{
#ifdef __OS2__
    if (ptr != NULL)
    {
...
__PDPCLIB_API__ void qsort ( base,nmemb,size,compar ) char *base ;  size_t nmemb ;  size_t size ;  int ( *compar) (  ) ;
{
...
__PDPCLIB_API__ void srand ( seed ) unsigned int seed ;
{
    myseed = seed;
    return;
}

__PDPCLIB_API__ int rand (  )
{
    int ret;
...
__PDPCLIB_API__ double atof ( nptr ) const char *nptr ;
{
    return (strtod(nptr, (char **)NULL));
}

__PDPCLIB_API__ double strtod ( nptr,endptr ) const char *nptr ;  char **endptr ;
{
    double x = 0.0;
    double xs= 1.0;
    double es = 1.0;
...
__PDPCLIB_API__ int atoi ( nptr ) const char *nptr ;
{
    return ((int)strtol(nptr, (char **)NULL, 10));
}

__PDPCLIB_API__ long int atol ( nptr ) const char *nptr ;
{
...
__PDPCLIB_API__ unsigned long int strtoul(nptr, endptr, base)
...
__PDPCLIB_API__ long int strtol ( nptr,endptr,base ) const char *nptr ;  char **endptr ;  int base ;
{
    unsigned long y;
    long x;
    int neg = 0;
...
__PDPCLIB_API__ int mblen ( s,n ) const char *s ;  size_t n ;
{
    if (s == NULL)
    {
        return (0);
...
__PDPCLIB_API__ int mbtowc ( pwc,s,n ) wchar_t *pwc ;  const char *s ;  size_t n ;
{
    if (s == NULL)
    {
        return (0);
...
__PDPCLIB_API__ int wctomb ( s,wchar ) char *s ;  wchar_t wchar ;
{
    if (s != NULL)
    {
        *s = wchar;
...
__PDPCLIB_API__ size_t mbstowcs ( pwcs,s,n ) wchar_t *pwcs ;  const char *s ;  size_t n ;
{
...
__PDPCLIB_API__ size_t wcstombs ( s,pwcs,n ) char *s ;  const wchar_t *pwcs ;  size_t n ;
{
...
#undef abs
#endif
__PDPCLIB_API__ int abs ( j ) int j ;
{
    if (j < 0)
    {
        j = -j;
...
__PDPCLIB_API__ div_t div ( numer,denom ) int numer ;  int denom ;
{
    div_t x;
...
#undef labs
#endif
__PDPCLIB_API__ long int labs ( j ) long int j ;
{
    if (j < 0)
    {
        j = -j;
...
__PDPCLIB_API__ ldiv_t ldiv ( numer,denom ) long int numer ;  long int denom ;
{
    ldiv_t x;

    x.l_quot = numer / denom;
    x.l_rem = numer % denom;
    return (x);
}

__PDPCLIB_API__ int atexit ( func ) void ( *func ) ( ) ;
{
    int x;
...
__PDPCLIB_API__ char *getenv ( name ) const char *name ;
{
#ifdef __OS2__
    PSZ result;
...
__PDPCLIB_API__ int system ( string ) const char *string ;
{
#ifdef __OS2__
    char err_obj[100];
    APIRET rc;
...
__PDPCLIB_API__ void *bsearch(key, base, nmemb, size, compar)
