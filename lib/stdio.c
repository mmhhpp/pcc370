...
#define CTYP
#endif
extern int CTYP __open();
extern int CTYP __read();
extern int CTYP __write();
extern void CTYP __seek();
extern void CTYP __close();
extern void CTYP __remove();
extern void CTYP __rename();
#endif
...
static int open(a,b,c) char*a; int b; int*c;
{
    int ret;
...
#endif
#ifdef __CMS__
extern void __SVC202 ( );
static int cmsrename();
static int cmsremove();
static char *int_strtok();
#define strtok int_strtok
#endif

int dbg_cond = 0;

__PDPCLIB_API__ int printf(format) char *format;
{
    va_list arg;
    int ret;
...
__PDPCLIB_API__ int fprintf(stream, format) FILE *stream; const char *format;
{
    va_list arg;
    int ret;
...
__PDPCLIB_API__ int vfprintf(stream, format, arg) FILE *stream; const char *format; va_list arg;
{
    int ret;
...
__PDPCLIB_API__ FILE *fopen(filename, mode) const char *filename; const char *mode;
{
#if defined(__VSE__)
    char *p;
    char *q;
...
__PDPCLIB_API__ int fclose(stream) FILE *stream;
{
#ifdef __OS2__
    APIRET rc;
#endif
...
static int vvprintf(format, arg, fq, s) const char *format; va_list arg; FILE *fq; char *s;
{
    int fin = 0;
    int vint;
    double vdbl;
...
static int examine(formt, fq, s, arg, chcount) const char **formt; FILE *fq; char *s; va_list *arg; int chcount;
{
    int extraCh = 0;
    int flagMinus = 0;
    int flagPlus = 0;
...
__PDPCLIB_API__ int fputc(c,stream) int c; FILE *stream;
{
    char buf[1];
...
#if !defined(__MVS__) && !defined(__CMS__)
__PDPCLIB_API__ int fputs(s, stream) const char *s; FILE *stream;
{
    size_t len;
    size_t ret;
...
__PDPCLIB_API__ int remove(filename) const char *filename;
{
    int ret;
#ifdef __OS2__
    APIRET rc;
...
__PDPCLIB_API__ int rename(old, newnam) const char *old; const char *newnam;
{
    int ret;
#ifdef __OS2__
    APIRET rc;
...
__PDPCLIB_API__ int sprintf(s, format) char *s; const char *format;
{
    va_list arg;
    int ret;
...
__PDPCLIB_API__ int vsprintf(s, format, arg) char *s; const char *format; va_list arg;
{
    int ret;
...
__PDPCLIB_API__ int ungetc(c, stream) int c; FILE *stream;
{
...
__PDPCLIB_API__ int fgetc(stream) FILE *stream;
{
    unsigned char x[1];
    size_t ret;
...
__PDPCLIB_API__ int fseek(stream, offset, whence) FILE *stream; long int offset; int whence;
{
    long oldpos;
    long newpos;
#ifdef __OS2__
...
__PDPCLIB_API__ long int ftell(stream) FILE *stream;
{
    return (stream->bufStartR + (stream->upto - stream->fbuf));
}

__PDPCLIB_API__ int fsetpos(stream, pos) FILE *stream; const fpos_t *pos;
{
    fseek(stream, *pos, SEEK_SET);
    return (0);
}

__PDPCLIB_API__ int fgetpos(stream, pos) FILE *stream; fpos_t *pos;
{
    *pos = ftell(stream);
    return (0);
}

__PDPCLIB_API__ void rewind(stream) FILE *stream;
{
    fseek(stream, 0L, SEEK_SET);
    return;
}

__PDPCLIB_API__ void clearerr(stream) FILE *stream;
{
    stream->errorInd = 0;
    stream->eofInd = 0;
    return;
...
__PDPCLIB_API__ void perror(s) const char *s;
{
    if ((s != NULL) && (*s != '\0'))
    {
        printf("%s: ", s);
...
__PDPCLIB_API__ int setvbuf(stream, buf, mode, size) FILE *stream; char *buf; int mode; size_t size;
{
    char *mybuf;
...
__PDPCLIB_API__ int setbuf(stream, buf) FILE *stream; char *buf;
{
    int ret;
...
__PDPCLIB_API__ FILE *freopen(filename, mode, stream) const char *filename;
                              const char *mode;
                              FILE *stream;
{
    inreopen = 1;
    ...
    dbg_cond = 0;
    fopen2();
    printf("* after fopen2 err %d dbg_cond 0x%x\n", err, dbg_cond);
...
__PDPCLIB_API__ int fflush(stream) FILE *stream;
{
#if defined(__MVS__) || defined(__CMS__)
    if ((stream->mode == __WRITE_MODE) && (stream->upto != stream->fbuf))
    {
...
__PDPCLIB_API__ char *tmpnam(s) char *s;
{
#if defined(__MVS__) || defined(__CMS__)
...
__PDPCLIB_API__ int fscanf(stream, format) FILE *stream; const char *format;
{
    va_list arg;
    int ret;
...
__PDPCLIB_API__ int scanf(format) const char *format;
{
    va_list arg;
    int ret;
...
__PDPCLIB_API__ int sscanf(s, format) const char *s; const char *format;
{
    va_list arg;
    int ret;
...
static int vvscanf(format, arg, fp, s) const char *format; va_list arg; FILE *fp; const char *s;
{
    int ch;
    int fin = 0;
    int cnt = 0;
...
__PDPCLIB_API__ char *gets(s) char *s;
{
    char *ret;
...
__PDPCLIB_API__ int puts(s) const char *s;
{
    int ret;
...
__PDPCLIB_API__ int getc(stream) FILE *stream;
{
    return (fgetc(stream));
}

__PDPCLIB_API__ int putc(c, stream) int c; FILE *stream;
{
    return (fputc(c, stream));
}

__PDPCLIB_API__ int getchar()
{
    return (getc(stdin));
}

__PDPCLIB_API__ int putchar(c) int c;
{
    return (putc(c, stdout));
}

__PDPCLIB_API__ int feof(stream) FILE *stream;
{
    return (stream->eofInd);
}

__PDPCLIB_API__ int ferror(stream) FILE *stream;
{
...
#if defined(__MVS__) || defined(__CMS__)
__PDPCLIB_API__ char *fgets(s, n, stream) char *s; int n; FILE *stream;
{
    unsigned char *eptr;
    size_t len;
    int cnt;
...
__PDPCLIB_API__ size_t fwrite(ptr, size, nmemb, stream) const void *ptr; size_t size; size_t nmemb; FILE *stream;
{
    size_t bytes;
    size_t sz;
    char *p;
...
__PDPCLIB_API__ size_t fread(ptr, size, nmemb, stream) void *ptr; size_t size; size_t nmemb; FILE *stream;
{
    size_t bytes;
    size_t read;
    size_t totalread;
...
static void filedef(fdddname, fnm, mymode) char *fdddname; char *fnm; int mymode;
{
...
static void fdclr(ddname) char *ddname;
{
    memset(&rb, 0x00, sizeof rb);
    rb.len = 20;
...
static void dblcvt(num, cnvtype, nwidth, nprecision, result) double num; char cnvtype; size_t nwidth; int nprecision; char *result;
{
    double b,round;
