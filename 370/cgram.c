...
adddef(){ /* add default case to switch */
	...
	deflab( swtab[swx].slab = getlab(), 1 );
	}
...
yyparse() {
 ...
 yystack:    /* put a state and value onto the stack */
	...
	if( yydebug  ) printf( "* yystate %d, yychar %d\n", yystate, yychar );
 ...
 yynewstate:
	...
	yyn = yypact[yystate];
	if ( yydebug ) printf ( "* yyn %d yystate %d\n", yyn, yystate );

	if( yyn<= YYFLAG ) goto yydefault; /* simple state */

	if( yychar<0 ) if( (yychar=yylex())<0 ) yychar=0;
	if ( yydebug ) printf ( "* after yylex1 yychar=%d\n", yychar );
	if( (yyn += yychar)<0 || yyn >= YYLAST ) goto yydefault;
	if (yydebug) printf("* checking for valid shift yyn=%d %d/%d\n", yyn, yychk[10], yyact[yyn]);
	if( yychk[ yyn=yyact[ yyn ] ] == yychar ){ /* valid shift */
		yychar = -1;
		yyval = yylval;
		yystate = yyn;
		if (yydebug) printf ("* yystate set to %d\n", yystate);
		if( yyerrflag > 0 ) --yyerrflag;
		goto yystack;
		}

 yydefault:
	if (yydebug) printf ("* yydefault state %d\n", yystate);
	/* default state action */

	if( (yyn=yydef[yystate]) == -2 ) {
		if( yychar<0 ) if( (yychar=yylex())<0 ) yychar = 0;
		if ( yydebug ) printf ( "* after yylex2 yychar=%d\n", yychar );
...
{  yypvt[-2].nodep->type = types( yypvt[-2].nodep->type, yypvt[-1].nodep->type, yypvt[-0].nodep->type );
			    yypvt[-1].nodep->op = FREE; yypvt[-0].nodep->op = FREE;
			    } break;
case 32:
