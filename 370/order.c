...
rallo( p, down ) register NODE *p; {
	...
	case ASG MOD:
		...
		if(fregs == 2 ){
			...
			down1 = NOPREF;  /* R1|MUSTDO; */
			down2 = NOPREF;
			break;
			}
		/* at least 3 regs free */
		/* compute lhs in (r0,r1), address of left in r2 */
		p->left->rall = NOPREF /* R1|MUSTDO */;
		mkrall( p->left, NOPREF /* R2|MUSTDO */ );
		/* now, deal with right */
		if( fregs == 3 ) rallo( p->right, NOPREF );
		else {
			/* put address of long or value here */
			p->right->rall = NOPREF /* R3|MUSTDO */;
			mkrall( p->right, NOPREF /* R3|MUSTDO  */);
			}
		return;
	...
	case MOD:
		...
		p->right->rall = NOPREF /* (fregs==3) ? R2|MUSTDO : R3|MUSTDO */ ;
		mkrall( p->right, NOPREF /* R2|MUSTDO */ );
		return;
...
offstar( p ) register NODE *p; {
	...
	if( p->op == PLUS || p->op == MINUS ){
		if( p->right->op == ICON  && p->left->op != REG){
...
deflab( l, code ){
	printf( "$L%05d  EQU  *   def2lab %d line %d\n", l, code, lineno );
	if (code) {
		printf( "          L    12,%d(,10)    def2lab2 line %d\n", pagno << 2, lineno );
		expands++;
		}
	}
...
argsize( p ) register NODE *p; {
	...
	else if( p->type == SHORT || p->type == USHORT ) {
		SETOFF( t, 4);
		return( t+4 );
		}
	else if( p->op == STARG ){
