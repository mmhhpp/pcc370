...
# ifdef MYREADER
	MYREADER(p);  /* do your own laundering of the input */
# endif
	if (expands >= EXPAND_LIMIT) {
		pagno ++;
		pages ++;
		l = getlab();
		printf ("          L    14,=A($L%05d) pager1\n", l);
		printf ("          BR   14\n");
		printf ("          DS   0F\n");
		printf ("          LTORG\n");
		printf ("          DS   0F\n");
		printf ("$L%05d EQU   *             pager2\n", l);
		printf ("          DROP 12\n");
		printf ("          BALR 12,0\n");
		printf ("          USING *,12\n");
		printf ("$P%05d  EQU  *    pager3\n", pages );
		expands = 0;
		}
	nrecur = 0;
...
delay1( p ) register NODE *p; {  /* look for COMOPS */
	...
	if (p->left->op != FREE && delay1(p->left))
		return( 1 );
	if (p->right->op != FREE && delay1(p->right))
		return( 1 );
	return( 0 );
...
codgen( p, cookie ) NODE *p; {
	...
	for(;;){
		...
		if (edebug) printf ("stotree: %o stocook %o\n", stotree, stocook);
...
cbranch( p, true, false ) NODE *p; {
	...
	case QUEST:
		...
		if( true < 0 ) deflab( tlab, 1);
		if( false < 0 ) deflab( flab, 1 );
		p->right->op = FREE;
		p->op = FREE;
		return;
...
oreg2( p ) register NODE *p; {
	...
	if( p->op == UNARY MUL ){
		...
		if( (q->op==PLUS || q->op==MINUS) && qr->op == ICON &&
			...
			ormake:
			if (edebug) printf("* ormake: %s: %o %d %d\n", opst[p->op], p, r, temp);
