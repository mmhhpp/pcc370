...
*/
	...
	for (;;) {
		if (mactop>inctop[ifno]) {/* retrieve hunk of pushed-back macro text */
			...
			maclvl=0;
			if (0<(ninbuf=fread(pbuf,1,BUFSIZ,fin))) {
...
#else
	if ( cp=strchr( filname, '.' ) ) *cp = 0;
	if( !strlen(filname) ) pperror( "null include file name" );
	strcpy( nfil, "dd:" );
	strcat( nfil, (inctype) ? "sysincl" : "include" );
	strcat( nfil, "(" );
	if( strlen(filname) > 8) filname[8] = 0;
	strcat( nfil, filname );
	strcat( nfil, ")" );
	if (0!=(fins[ifno+1]=fopen(nfil,"r"))) {
		filok=1; fin=fins[++ifno];
	}
#endif
...
dodef(p) char *p; {/* process '#define' */
	...
	/* fprintf(stderr, "savch %x sbf %x SBSIZE %d BUFSIZ %d result %d\n", savch, sbf, SBSIZE, BUFSIZ, savch-(sbf+SBSIZE-BUFSIZ)); */
